...
'''
    ...
    for i in range(26):
        dict_2[aa[i]]=real_key[i]

    # retrieve original plain text from original key


    original_plain_text = ''
    _keys_list = list(dict_2.keys())
    _values_list = list(dict_2.values())
    for c in ciphertext:
        idx = _values_list.index(c)
        original_plain_text = original_plain_text + _keys_list[idx]

    # original_plain_text = ''
    # _orig_keys_list = list(dict_2.keys())
    # _orig_values_list = list(dict_2.values())
    # for c in ciphertext:
    #     idx = _orig_values_list.index(c)
    #     original_plain_text = original_plain_text + _orig_keys_list[idx]

    print("original text with original key:   " + original_plain_text)

    #get percentage match between original plain text and decrypted plaintext

    plaintext_match_count = 0

    for i in range(len(original_plain_text)):
        if final_plain_text[i] is original_plain_text[i]:
            plaintext_match_count += 1

    print(plaintext_match_count)
    print(len(original_plain_text))



    # for c in original_plain_text:
    #     if
    #     temp = final_plain_text[index]
    #     if c in temp:
    #         plaintext_match_count += 1
    #     index += 1

    text_match = ((plaintext_match_count / len(original_plain_text))*100)
    print(text_match)
