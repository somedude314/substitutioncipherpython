...
def getFrequencyCounts(rawData):
    ...
    frequency_list, frequency_char_list = zip(*sorted(zip(frequency_list, frequency_char_list), reverse=True)) ###explain
...
def getBigram(rawData, freq_char_list):
    ...
    for entry in list_of_2_chars:
        char1_idx = freq_char_list.index(entry[0]) ###index after sorting from most frequent
...
def clean_up(inputtxt):    #didnt we clean up the code in decryption UI? is this necessary?
